document.addEventListener('DOMContentLoaded', async () => {
  ...
  if (!isLoggedIn()) {
    try {
      const response = await fetch('https://travelbackend-x5tx.onrender.com/logout', {
        method: 'GET',
        credentials: 'include'
      });
      if (response.ok) {
  ...
  if (signupForm) {
    signupForm.addEventListener('submit', async (e) => {
      ...
      try {
        const response = await fetch('https://travelbackend-x5tx.onrender.com/signup', {
          method: 'POST',
          headers: {
  ...
  if (loginForm) {
    loginForm.addEventListener('submit', async (e) => {
      ...
      try {
        const response = await fetch('https://travelbackend-x5tx.onrender.com/login', {
          method: 'POST',
          headers: {
            'Content-Type': 'application/json'
          },
